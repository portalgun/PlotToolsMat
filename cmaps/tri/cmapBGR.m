function cm = cmapBGR(m, n, interp)
%cmapBGR: symmetric/diverging/bipolar colormap, with neutral central color.
%
% Usage: cm = cmapBGR(m, neutral, interp)
%  neutral is the gray value for the middle of the colormap, default 1/3.
%  m:     number of rows in the colormap, defaulting to copy the current
...
%  colormap(cmapBGR(201, 0.1)) % dark gray as neutral
%  axis off; colorbar
%  pause(2)
%  colormap(cmapBGR(201, 0.9)) % light gray as neutral
%  pause(2)
%  colormap(cmapBGR(201, 0.5)) % grayscale-friendly colormap
%
% See also: colormap, jet, interp1, colormap_investigation, dusk
% dusk is a colormap like cmapBGR(m, 0.5), in Oliver Woodford's real2rgb:
...
elseif n > 0.5
    ...
    if ~strcmp(interp, 'linear')
        warning('cmapBGR:nonlinearluminance', ...
